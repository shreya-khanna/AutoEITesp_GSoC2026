...
Speechmatics's transcriptions (Enhanced) -- for review/reporting, sourced from the
data/postprocessed/resegmented/{version}/{recording}/transcriptions.csv files that
build_postprocessed_csv_resegmented.py regenerates once "speechmatics" is in its ASR_SOURCES.

Scoped to hit recordings only (data/resegmented_hit_recordings.txt), since speechmatics_text is
only populated for those (see transcribe_resegmented_speechmatics.py). WER/MER/CER are not in this
sheet by design (it's meant to stay a plain 4-column reviewable transcript) -- they're already
computed as speechmatics_wer/mer/cer_{rater1,rater2} columns in the source CSVs, which
notebooks/model_comparison/analyze_speechmatics_transcription_quality.ipynb reads directly.

Usage:
    python -m src.postprocessing.build_speechmatics_excel
"""
import csv
import logging
...
COLUMNS = ["stimuli", "rater1 transc.", "rater2 transc.", "Speechmatics's transcriptions"]
            ...
            with open(csv_path, newline="", encoding="utf-8") as f:
                for row in csv.DictReader(f):
                    ws.append([
                        ...
                        row.get("speechmatics_text"),
                    ])
                    n_rows += 1

    out_path = data_path("postprocessed", "speechmatics_transcriptions.xlsx")
    wb.save(out_path)
    log.info(
