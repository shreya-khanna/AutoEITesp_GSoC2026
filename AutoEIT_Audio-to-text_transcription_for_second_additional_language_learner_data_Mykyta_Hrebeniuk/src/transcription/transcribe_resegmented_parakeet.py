...
src/preprocessing/resegment_window.py) with NVIDIA's Parakeet-TDT-0.6B-v3, reusing
transcribe_parakeet.py's load_model()/transcribe_file().

Output goes to data/transcribed/resegmented/parakeet/{version}/{recording}/item_{N}.json, where N
...
Usage:
    python -m src.transcription.transcribe_resegmented_parakeet
"""
import json
import logging

from src.transcription.resegmented_items import VERSIONS, iter_resegmented_items
from src.transcription.transcribe_parakeet import load_model, transcribe_file
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(data_path("logs", "transcribe_resegmented_parakeet.log")),
...
def main() -> None:
    ...
    for version in VERSIONS:
        ...
        for recording, index, audio_path in items:
            output_path = data_path("transcribed", "resegmented", "parakeet", version, recording, f"item_{index}.json")
