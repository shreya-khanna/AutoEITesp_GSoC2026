...
src/preprocessing/resegment_window.py) with jonatasgrosman/wav2vec2-large-xlsr-53-spanish (CTC),
reusing transcribe_wav2vec2.py's load_model()/transcribe_file().

Output goes to data/transcribed/resegmented/wav2vec2/{version}/{recording}/item_{N}.json, where N
...
Usage:
    python -m src.transcription.transcribe_resegmented_wav2vec2
"""
import json
import logging

from src.transcription.resegmented_items import VERSIONS, iter_resegmented_items
from src.transcription.transcribe_wav2vec2 import load_model, transcribe_file
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(data_path("logs", "transcribe_resegmented_wav2vec2.log")),
...
def main() -> None:
    ...
    for version in VERSIONS:
        ...
        for recording, index, audio_path in items:
            output_path = data_path("transcribed", "resegmented", "wav2vec2", version, recording, f"item_{index}.json")
