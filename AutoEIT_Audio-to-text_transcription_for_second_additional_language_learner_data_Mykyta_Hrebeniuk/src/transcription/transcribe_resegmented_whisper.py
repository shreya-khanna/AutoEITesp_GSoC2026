...
src/preprocessing/resegment_window.py) with the same faster-whisper large-v3 setup as
transcribe_v2.py (language forced to "es"), reusing its load_model()/transcribe_file().

Output goes to data/transcribed/resegmented/whisper/{version}/{recording}/item_{N}.json, where N
...
Usage:
    python -m src.transcription.transcribe_resegmented_whisper
"""
import json
import logging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(data_path("logs", "transcribe_resegmented_whisper.log")),
...
def main() -> None:
    ...
    for version in VERSIONS:
        ...
        for recording, index, audio_path in items:
            output_path = data_path("transcribed", "resegmented", "whisper", version, recording, f"item_{index}.json")
